...
(function () {
  ...
  const STORAGE_KEYS = {
    ...
    TIMESTAMPS: 'tabnest_timestamps', // lastActiveTimestamp map
  };

  /** Default lifecycle timer settings */
  const DEFAULT_SETTINGS = {
    t1Minutes: 5,          // Active → Discarded
    t2Minutes: 15,         // Discarded → Saved & Closed
    t3Days: 7,             // Saved → Archived
    autoGroup: true,
    persistSessions: true,
    managePinned: false,   // If true, pinned tabs can be discarded (but never closed)
    hoverPreRender: true,
    showRamSavings: true,
    batchSize: 3,          // Staggered restore batch size
    whitelist: [],         // User-whitelisted domains (Stage 2 max)
  };
  ...
  /**
   * Domain dictionary — 100+ known domains mapped to group IDs.
   ...
   *
   * Keywords are matched as case-insensitive substrings against
   * the URL path + tab title combined.
   */
  const KEYWORD_SETS = {
    dev: [
